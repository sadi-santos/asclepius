...
function Get-TreeView([string]$root,[string]$title){
  ...
  if(-not (Test-Path $root)){ Write-Line "   (nA£o encontrado) $root" "Yellow"; return }
...
function Test-Frontend([string]$root){
  ...
  if(-not $pkg){
    Add-ReportItem "frontend" "Error" "package.json nA£o encontrado" "Rode na pasta do frontend ou ajuste -FrontendPath."
    return
  }
  ...
  $ts = Read-Json $tsPath
  if(-not $ts){
    Add-ReportItem "frontend/tsconfig.json" "Error" "tsconfig.json nA£o encontrado" "Criar tsconfig e incluir 'vitest/globals' em compilerOptions.types."
  } else {
    $types = @()
    if($ts.compilerOptions -and $ts.compilerOptions.types){ $types = @($ts.compilerOptions.types) }
    if(-not ($types -contains "vitest/globals")){
      Add-ReportItem "frontend/tsconfig.json" "Warn" "types nA£o contA©m 'vitest/globals'" "Adicionar 'vitest/globals' em compilerOptions.types."
      if($Fix){ $ts.compilerOptions.types = (@($types) + "vitest/globals" | Select-Object -Unique) }
    }
    if($types -contains "@playwright/test"){
      Add-ReportItem "frontend/tsconfig.json" "Warn" "types contA©m '@playwright/test' (pode conflitar com Vitest)" "Remover '@playwright/test' de compilerOptions.types."
  ...
  if(-not (Test-Path $vitestCfg)){
    Add-ReportItem "frontend" "Warn" "vitest.config.ts ausente" "Criar config bA¡sico com jsdom e tsconfig paths."
    if($Fix){
      $cfg = @'
...
'@
  ...
  if(@($unitCandidates).Count -eq 0){
    Add-ReportItem "frontend/tests" "Warn" "teste unitA¡rio de CPF nA£o encontrado" "Criar tests/diagnostics/unit/cpf.test.ts (ou manter cpf.spec.ts existente)."
    if($Fix){
      $unitPath = Join-PathEx $root "tests/diagnostics/unit/cpf.test.ts"
      $spec = @'
import { onlyDigits, formatCPFView, validateCPF } from "@/utils/cpf";

describe("utils/cpf (diagnA³stico)", () => {
  it("onlyDigits", () => {
  ...
  it("validateCPF vA¡lidos conhecidos", () => {
    expect(validateCPF("52998224725")).toBe(true);
    expect(validateCPF("12345678909")).toBe(true);
  });
  it("validateCPF invA¡lidos", () => {
...
'@
  ...
  if(@($e2eCfgPaths).Count -eq 0){
    Add-ReportItem "frontend/e2e" "Warn" "playwright.config.ts de E2E nA£o encontrado" "Criar em tests/diagnostics/e2e/playwright.config.ts."
    if($Fix){
...
'@
  ...
  if(@($e2eSpecs).Count -eq 0){
    Add-ReportItem "frontend/e2e" "Warn" "nenhum *.e2e.spec.ts encontrado" "Criar spec para o fluxo de ediA§A£o sem tocar no CPF."
  }

  # utils/cpf.ts
  $utilsCandidates = @( Get-ChildItem -Recurse -File -Path $root -Filter "cpf.ts" )
  if(@($utilsCandidates).Count -eq 0){
    Add-ReportItem "frontend/utils" "Error" "src/utils/cpf.ts nA£o encontrado" "Crie o mA³dulo com onlyDigits/formatCPFView/validateCPF."
  } else {
  ...
  # PatientForm (validar CPF sA³ quando alterado)
  $pathsToCheck = @(
  ...
  if(@($formPaths).Count -eq 0){
    Add-ReportItem "frontend/PatientForm" "Warn" "PatientForm.tsx nA£o encontrado" "Confirme o caminho do formulA¡rio."
  } else {
    ...
    if(-not $hasRef -or -not $hasAlter -or -not $validOnChange){
      Add-ReportItem "frontend/PatientForm" "Warn" "Regra 'validar CPF sA³ quando alterado' nA£o detectada" "Aplicar patch do PatientForm."
    } else {
      Add-ReportItem "frontend/PatientForm" "Info" "Regra de validaA§A£o condicional de CPF detectada" ""
    }
  }
}
...
function Test-Backend([string]$root){
  if(-not (Test-Path $root)){ Add-ReportItem "backend" "Error" "DiretA³rio do backend nA£o encontrado" ""; return }
  ...
  try{
    ...
    if($digits -ne 11){
      Add-ReportItem "backend/api" "Warn" "GET /patients/$id retorna CPF com $digits dA­gitos" "Padronize o retorno do backend como 11 dA­gitos."
    } else {
      Add-ReportItem "backend/api" "Info" "GET /patients/$id retorna CPF com 11 dA­gitos" ""
    }
  } catch {
...
# ---------- EXECUA‡AƒO ----------
Write-Line "ðY"¦ Auditor de Projeto Asclepius" "Cyan"
...
Write-Line ("`nRelatA³rio salvo em: " + (Resolve-Path $outPath).Path) "Green"

if($Fix){
  Write-Line "`nas™i¸  Modo -Fix aplicado para itens simples (config/testes). Revise o diff no Git." "Yellow"
}
